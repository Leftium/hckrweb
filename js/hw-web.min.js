...
}function d(a){s=[],t=[];var b=Math.floor(new Date/1e3),d=amplify.store("last_visit")||b,e=amplify.store("num_visits")||0;console.log("before",e,d),e++,amplify.store("last_visit",b),amplify.store("num_visits",e),console.log("after",e,d);for(var f=0;f<a.length&&a[f].date>d;){var g=c(a[f]);f++,g.i=f,s.push(g)}for(s.push({visit_count:e,time_ago:relativeDate(1e3*d),type:"visit"});f<a.length;){var g=c(a[f]);f++,g.i=f,t.push(g)}}var e=function(){return+new Date},f=!!a.XDomainRequest,g="withCredentials"in new XMLHttpRequest||f,h=!1,i=2e4,j={};
try{h=new Worker("js/hnapi-worker.js"),h.addEventListener("message",function(a){var b=a.data,c=b.url||"";if(j[c]){var d=j[c],e=d.error,f=d.success;delete j[c],b.error?e(b.error):f(b.response)}},!1)}catch(k){}var l=function(b,c,d){if(c||(c=function(){}),d||(d=function(){}),g)if(h)j[b]={success:c,error:d},h.postMessage({url:b,timeout:i});else{var k=j[b]||(f?new XDomainRequest:new XMLHttpRequest);k._timeout&&clearTimeout(k._timeout),k._timeout=setTimeout(function(){k.abort()},i),k.onload=function(){clearTimeout(this._timeout),delete j[b];
try{c(JSON.parse(this.responseText))}catch(a){d(a)}},k.onerror=k.onabort=k.ontimeout=function(a){clearTimeout(this._timeout),delete j[b],d(a)},(k.readyState<=1||f)&&(k.open("GET",b+"?"+e(),!0),k.send()),j[b]=k}else{var l=a.document,m=l.createElement("script"),n="callback"+e();a[n]=c,m.onerror=d,m.src=b+"?callback="+n,l.body.appendChild(m)}},m=["//node-hnapi-eu.herokuapp.com/","//node-hnapi.azurewebsites.net/","//node-hnapi-asia.azurewebsites.net/","//node-hnapi-eus.azurewebsites.net/","//node-hnapi-weu.azurewebsites.net/","//node-hnapi-wus.azurewebsites.net/","//node-hnapi-ncus.azurewebsites.net/"],n=function(a){for(var b=a.length-1;b>0;b--){var c=Math.floor(Math.random()*(b+1)),d=a[b];
