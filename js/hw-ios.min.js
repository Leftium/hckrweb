...
domain:a.source,points:a.points,user:a.submitter,time_ago:relativeDate(1e3*a.date),time:a.date,comments_count:a.comments,type:"link"}}function c(a){r=[];var c={visit_count:u,time_ago:relativeDate(1e3*t),type:"visit"};for(i=0;i<a.length;i++){v>t&&t>=a[i].date&&r.push(c),v=a[i].date;var d=b(a[i]);d.i=i+1,r.push(d)}}var d=function(){return+new Date},e=!!a.XDomainRequest,f="withCredentials"in new XMLHttpRequest||e,g=!1,h=2e4,j={};try{g=new Worker("js/hnapi-worker.js"),g.addEventListener("message",function(a){
